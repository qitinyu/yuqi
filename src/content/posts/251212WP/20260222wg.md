...
**问题**：直接在 CMD 中运行 cloudflared，关闭窗口或关机后隧道断开。

**解决**：将 cloudflared 注册为 Windows 系统服务，开机自动后台运行。

### 同时确保 OpenList Desktop 也开机自启
...
**2. 注册自启动服务**
创建好上面的 .vbs 文件后，我们需要把它放入 Windows 的启动文件夹中。
- 按键盘上的 <kdb>Win + R </kdb>键。
- 输入 `shell:startup` 并回车。
- 将你刚才创建的 `start.vbs` 文件复制（或剪切）到这个打开的文件夹中。
